# The training part of GCN, the trained model is saved in the model folder.
            ...
            if i==1:
                for j in range(1052):
                    if (j+1>=28 and j+1<=275) or (j+1>=287 and j+1<=386) or(j+1>=425 and j+1<=771) or(j+1>=800 and j+1<=1052):
                        name='./Cluster_feature_10_42D/'+str(j+1)+'.mat'
                        try:
...
#                            continue
            ...
            if i>=2 and i<=9:
                for j in range(1052):
                    if (j+1>=1 and j+1<=27+(i-2)*27)or(j+1>=276 and j+1<=286+(i-2)*11)or(j+1>=387 and j+1<=424+(i-2)*38)or(j+1>=772 and j+1<=799+(i-2)*28):
                        name='./Cluster_feature_10_42D/'+str(j+1)+'.mat'
                        try:
                            data = sio.loadmat(name)
                            nodes_feature=data['Cluster_features']  
...
#                            continue
                    ...
                    if (j+1>=55+(i-2)*27 and j+1<=275)or(j+1>=298+(i-2)*11 and j+1<=386)or(j+1>=463+(i-2)*38 and j+1<=771)or(j+1>=828+(i-2)*28 and j+1<=1052):
                        name='./Cluster_feature_10_42D/'+str(j+1)+'.mat'
                        try:
                            data = sio.loadmat(name)
                            nodes_feature=data['Cluster_features']  
...
#                            continue
            ...
            if i>=2 and i<=9:
                for j in range(1052):
                    if (j+1>=28+(i-2)*27 and j+1<=54+(i-2)*27)or(j+1>=287+(i-2)*11 and j+1<=297+(i-2)*11)or(j+1>=425+(i-2)*38 and j+1<=462+(i-2)*38)or(j+1>=800+(i-2)*28 and j+1<=827+(i-2)*28):
                        name='./Cluster_feature_10_42D/'+str(j+1)+'.mat'
                        try:
                            data = sio.loadmat(name)
                            nodes_feature=data['Cluster_features']  
...
#                            continue
            ...
            if i==10:
                for j in range(1052):
                    if (j+1>=244 and j+1<=275)or(j+1>=375 and j+1<=386)or(j+1>=729 and j+1<=771)or(j+1>=1024 and j+1<=1052):
                        name='./Cluster_feature_10_42D/'+str(j+1)+'.mat'
                        try:
                            data = sio.loadmat(name)
                            nodes_feature=data['Cluster_features']  
...
class model_3DGCN(nn.Module):
    ...
    def forward(self,x,adj):
        ...
        for i in range(self.num_layers):
            feature_update=self.GATLayerAdj(feature_update, adj)
#            if i==0:
#                feature_concatenation=feature_update.unsqueeze(0)
#            else:
#                feature_concatenation=torch.cat([feature_concatenation,feature_update.unsqueeze(0)],dim=2)
#        feature_concatenation=feature_concatenation.squeeze(0)
...
for fold in range(1,11):
    ...
    for epoch in range(70):
        ...
        pkl_path='./model/'+str(fold)+'/net'+str(epoch)+'.pkl'
...
#    plt.show()
    ...
    for iter, (test_bg, test_adj, test_label) in enumerate(test_data_loader):
        ...
        prediction=model(test_bg,test_adj)
